...
void H265Codec::forceSoftwareEncoding(bool force) noexcept
{
    auto registry = Gst::Registry::get();
    auto soft = registry->lookup_feature("x265enc");
    if (soft)
    {
        soft->set_rank(force ? Gst::RANK_PRIMARY : Gst::RANK_SECONDARY);
    }

    auto hard = registry->lookup_feature("vaapih265enc");
    if (hard)
    {
...
const char* H265Codec::getMimeType() const noexcept
{
    return "video/x-h265";
}

void H265Codec::configureElement(const Glib::ustring& factoryName, const Glib::RefPtr<Gst::Element>& element) const
{
    const int bitrate =
    ...
    if (factoryName == "x265enc")
    {
        ...
        case EncodingMode::bitrate:
            ...
            element->set_property("qp", -1);
            break;

        case EncodingMode::quality:
            element->set_property("bitrate", defaultBitrate);
            element->set_property("qp", qp);
            break;
        }
    }
    else if (factoryName == "vaapih265enc")
    {
